    ...
    ->Array.filterMap(line => {
      ...
      if line === "" || line->String.startsWith("#") {
        ...
        Some({
          ...
          pattern: getField(1),
          flags: getField(2),
          captureKind: getField(3),
          captureValue: getField(4),
          cardinality: getField(5),
          source: getField(6),
          resultKind: getField(7),
          resultValue: getField(8),
        })
      }
    })
}
...
describe("named generator integration", () => {
  ...
  testAsync("commits named files transactionally and removes stale owned output", async () => {
    ...
    let removedPpxConfigPath = Path.join([output, "rescript-embed-lang.json"])
    ...
    try {
      ...
      ok(!Fs.existsSync(removedPpxConfigPath), ~message="removed PPX config should not be written")
      ...
      ok(
        !Fs.existsSync(removedPpxConfigPath),
        ~message="removed PPX config should not be written",
      )
      equal(
        userOwned->Fs.readFileSync->NodeJs.Buffer.toString,
        "let userFile = true\n",
