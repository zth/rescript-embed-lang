...
describe("named generator integration", () => {
  ...
  testAsync("commits named files transactionally and removes stale owned output", async () => {
    ...
    try {
      ...
      ok(
        alphaContent->String.includes(
          "module SourceHash_e054f1d1ccc42ed629f1850f9b472e202bc727038ad015c4b7b13e5a165688f2",
        ),
        ~message="exact source-hash module missing",
      )
...
describe("sequential generator compatibility", () => {
  open NamedGenerationFixture
  open NamedGeneratorIntegration

  testAsync("adopts legacy generated files and preserves M1 layout", async () => {
    let root = mkdtempSync(Path.join([NodeJs.Os.tmpdir(), "rescript-embed-lang-legacy-"]))
    let src = Path.join([root, "src"])
    let output = Path.join([root, "generated"])
    Fs.mkdirSync(src)
    Fs.mkdirSync(output)
    let source = Path.join([src, "Legacy.res"])
    write(source, "let legacy = %generated.legacy(\x60legacy source\x60)\n")
    let legacyGenerated = Path.join([output, "Legacy__legacy.res"])
    let legacyExtra = Path.join([output, "Legacy__legacy.M1.css"])
    write(legacyGenerated, "// @sourceHash old\n\nmodule M1 = {\n  let default = 0\n}\n")
    write(legacyExtra, "old css\n")

    let embed = RescriptEmbedLang.make(
      ~extensionPattern=Generic("legacy"),
      ~setup=RescriptEmbedLang.defaultSetup,
      ~generate=async ({emitExtraFile}) => {
        ignore(emitExtraFile(~extension="css", ~content="new css\n", ~moduleName=None))
        Ok(NoModuleName({content: "let default = 1"}))
      },
      ~cliHelpText="legacy",
    )

    try {
      await RescriptEmbedLang.runCli(embed, ~args=["generate", "--src", src, "--output", output])
      let generatedContent = legacyGenerated->Fs.readFileSync->NodeJs.Buffer.toString
      ok(generatedContent->String.includes("module M1"), ~message="legacy M1 layout changed")
      ok(
        generatedContent->String.startsWith("// @generated by rescript-embed-lang v1"),
        ~message="legacy output was not adopted",
      )
      equal(
        legacyExtra->Fs.readFileSync->NodeJs.Buffer.toString,
        "new css\n",
        ~message="legacy extra artifact was not adopted",
      )
      ok(
        Fs.existsSync(Path.join([output, ".rescript-embed-lang-legacy.json"])),
        ~message="ownership index was not written",
      )
    } catch {
    | JsExn(error) =>
      rmSync(root, {recursive: true, force: true})
      panic(error->JsExn.message->Option.getOr("legacy integration test failed"))
    }
    rmSync(root, {recursive: true, force: true})
  })
})
